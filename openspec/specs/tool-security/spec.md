...
工具执行安全能力为 QueryLoop / ToolRunner 提供 bash 命令沙箱、插件化工具注册表、并发执行隔离与权限风险分级。实现于 `internal/layers/contextengine/enforce/tools/` 与 `internal/shared/config/tool_config.go`。
