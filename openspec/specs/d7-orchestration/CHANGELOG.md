...
| 2026-07-01 | devrix-d7-physical-layout-alignment | PR-1+PR-2（纯 markdown + Go layout guard 子包，0 业务代码行为变更）：PR-1 a-registry v5.1.0→v5.4.0 + f-registry v5.1.0→v5.4.0 + code-layout v1.12.0→v1.13.0（Hardening 段 + D7-X Cross-S Kernel 段 + §4.2 D7 终态化）；PR-2 layout/ 子包 4 .go + 6 ghost 行 status flip；PR-3 a-registry v5.4.0→v5.5.0 + code-layout v1.13.0→v1.14.0（D7-S6-A03/A04 PlanValidate/PlanGenerate 双登记 + S5 双登记说明段）；t-registry v4.19.0→v4.22.0 + 根 t-registry v5.11.0→v5.13.0 | PROPOSED | [changes](../../changes/devrix-d7-physical-layout-alignment/) |
