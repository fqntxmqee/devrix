...
| **D7-S14-A50-T12** | ResumeSession T2 续跑入口 (ProcessMessage 开头检查 → applyResumeSession) + applyResumeSession (user_choice=A→runLoopWithResume Continue / B→ForceExit / C→AbortWithAudit + 补写 audit) + runLoopWithResume (depth 续 T1 状态由 LoopDepthTracker 自动保证) | **PARTIAL** | `orchestration/escape/arbitrator.go` (PR-V5.3) + 后续 PR-V5.6 接入 SessionOrchestrator |
...
│   ├── T12  ResumeSession + applyResumeSession + runLoopWithResume               [PARTIAL]
...
**Total**: 17 IMPLEMENTED P0 T points, 1 PARTIAL (T12 ResumeSession T2 续跑 SessionOrchestrator 接入待 PR-V5.6), 0 PLANNED.
