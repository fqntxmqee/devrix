...
| **D7-S3-A50-T01** | **`DAGExecutor` interface (input: PlanDAG, output: readySet → WorkerRunner)** | **D7-S3-A50** | **`wavescheduler/dag_executor.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Wave_DAG |
| **D7-S3-A50-T02** | **`RunPlanDAG` 4-worker pool (固定大小 = 4, 4 worker 并行执行 readySet 节点)** | **D7-S3-A50** | **`wavescheduler/dag_executor.go` + `dag_executor_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Wave_DAG |
| **D7-S3-A50-T03** | **`readySet` priority heap (按 Priority 排序, 同 Priority 按 ID 字典序)** | **D7-S3-A50** | **`wavescheduler/dag_executor.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Wave_DAG |
| **D7-S3-A50-T04** | **任一 worker error → cancel sibling + abort DAG (context.Cancel 串行传播)** | **D7-S3-A50** | **`wavescheduler/dag_executor.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Wave_DAG |
| **D7-S3-A50-T05** | **`DAGExecutor` routing flag (`devrix.d7.dag_executor.enabled`, flag off 走 legacy WaveScheduler)** | **D7-S3-A50** | **`wavescheduler/dag_executor.go` + `sessionorchestrator/item_pipeline.go::runExecuteStage`** | **IMPLEMENTED (DM-20260707-001 PR-B + PR-D)** | **P0** | Wave_DAG |
...
| **D7-S5-A50-T01** | **`IntentSegment` 5 字段类型 (ID/Text/Kind/Confidence/SourceObservationIDs) + Validate + sealed Payload + 不可变** | **D7-S5-A50** | **`orchtypes/intent_segment.go` + `orchtypes/intent_segment_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-A1)** | **P0** | Plan_DAG |
| **D7-S5-A50-T02** | **`IntentSegmentSet` 容器 + Add/Drain/Validate + 防御性拷贝** | **D7-S5-A50** | **`orchtypes/intent_segment_set.go` + 测试** | **IMPLEMENTED (DM-20260707-001 PR-A1)** | **P0** | Plan_DAG |
| **D7-S5-A50-T03** | **`PlanDAG` + `PlanNode` + `DataEdge` + Priorities + `validateDAG` 4 子错误 (cycle / dangling dep / dup id / zero priority)** | **D7-S5-A50** | **`plan/dag.go` + `plan/dag_validator.go` + `plan/dag_validator_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-A1)** | **P0** | Plan_DAG |
| **D7-S5-A50-T04** | **方案 β: `plan.Plan` 加 `IntentSegmentSet` + `DAG` 2 可选字段, `SpawnPolicy` 完全不动 (3 值字符串枚举 SpawnNone/SpawnDecompose/SpawnInline)** | **D7-S5-A50** | **`plan/plan_struct.go`** | **IMPLEMENTED (DM-20260707-001 PR-A1)** | **P0** | Plan_DAG |
| **D7-S5-A50-T16** | **`devrix.d7.dag_executor.enabled` config flag (default false, 5%→100% 灰度)** | **D7-S5-A50** | **`orchtypes/config.go` + `bootstrap/wire_coordinator.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Plan_DAG |
| **D7-S5-A51-T01** | **`AcceptanceCriterion` struct + 8 `CheckKind` 枚举 (file_exists / file_contains / regex_match / tool_returns / json_path / http_status / line_count / custom_llm_judge)** | **D7-S5-A51** | **`plan/acceptance_criteria.go` + `plan/acceptance_criteria_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T02** | **`PerCriterionVerdict` struct + 4 `Outcome` 枚举 (pass / fail / partial / error)** | **D7-S5-A51** | **`plan/acceptance_criteria.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T03** | **aggregation 4 路径: all_pass / any_fail / weighted_score ≥ threshold / custom_llm_judge override (priority ordered)** | **D7-S5-A51** | **`plan/acceptance_aggregator.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T04** | **`PlanAcceptanceContractBuilder` 从 `StrategicPlanProposal.DAG` 节点构造 AC[] (per-node min 1 AC, file_exists default)** | **D7-S5-A51** | **`plan/contract_builder.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T05** | **`PlanLLMIO` JSON schema (plan_kind + dag + ac[] + source_observation_ids + retry_feedback)** | **D7-S5-A51** | **`plan/plan_llm_io.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T06** | **`VerifyLLMIO` JSON schema (per_criterion_verdict[] + aggregate + confidence + explanation)** | **D7-S5-A51** | **`plan/verify_llm_io.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T07** | **`PlanParseReject` 3 子类 (parse_error / schema_violation / llm_refusal) + 反馈到 LLM retry loop** | **D7-S5-A51** | **`plan/plan_parse_reject.go` + 测试** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T08** | **`CustomLLMJudge` budget ≤ 3 (semaphore 串行复用 plan LLM)** | **D7-S5-A51** | **`plan/custom_llm_judge.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T09** | **7 CheckKind 本地执行 (file_exists / file_contains / regex_match / tool_returns / json_path / http_status / line_count) 0 LLM call** | **D7-S5-A51** | **`plan/checker.go` + 7 子实现** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T10** | **AC feedback loop: VerifyLLMIO.aggregate=fail → Plan retry with rejected_criteria[] + retry_attempt** | **D7-S5-A51** | **`plan/feedback_loop.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T11** | **`PlanParseReject` 2 sentinel error (ErrPlanParseReject + ErrVerifyAggregateFail)** | **D7-S5-A51** | **`plan/errors.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T12** | **AcceptanceCriterion.Validate fail-fast (empty ID / empty description / unsupported CheckKind)** | **D7-S5-A51** | **`plan/acceptance_criteria.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T13** | **PerCriterionVerdict aggregation order: fail > partial > error > pass (precedence)** | **D7-S5-A51** | **`plan/acceptance_aggregator.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T14** | **CustomLLMJudge retry-on-error: LLM 调用失败时 downgrade 到 weighted_score 路径** | **D7-S5-A51** | **`plan/custom_llm_judge.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A51-T15** | **PlanLLMIO retry feedback 字段 (retry_attempt + rejected_criteria[]) ≤ 5 retries 上限** | **D7-S5-A51** | **`plan/plan_llm_io.go`** | **IMPLEMENTED (DM-20260707-001 PR-A2)** | **P0** | Plan_AC |
| **D7-S5-A70-T01** | **`PlanFieldValidator` 10 字段级子类拒绝 (steps_empty / unknown_field / kind_mismatch / deps_missing / source_observation_ids_empty / ac_empty / channel_invalid / priority_zero / retry_budget_exceeded / dag_invalid)** | **D7-S5-A70** | **`plan/plan_field_validator.go` + `plan/plan_field_validator_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T02** | **`PlanParseReject` 6 扩展子类 (steps_empty / unknown_field / kind_mismatch / deps_missing / dag_invalid / ac_invalid) — 与 A51 3 子类合并 12 错误码** | **D7-S5-A70** | **`plan/plan_parse_reject.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T03** | **`RetryWithFeedback` ≤ 2 retries (PlanFieldValidator 可恢复错误走 retry + PlanParseReject.feedback 注入)** | **D7-S5-A70** | **`plan/plan_retry_decompose.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T04** | **`DecomposeIntoChildren` fallback (StepsEmpty / DAGInvalid / parse AST error → child WorkItem 拆分)** | **D7-S5-A70** | **`plan/plan_retry_decompose.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T05** | **`PlanErrorDecision` abort+NO_Learn (KindUnset / SourceObservationIDsEmpty → ActionAbort 不触发 Learn reputation)** | **D7-S5-A70** | **`plan/plan_error_decision.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T06** | **Decision 表 10→11 行 plan_error 映射 (PlanErrorRouteFor 3-tier precedence: exact → family → catch-all)** | **D7-S5-A70** | **`plan/plan_error_decision.go` + `sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
| **D7-S5-A70-T07** | **`force_plan` Plan injection 跨包 contract (Plan 节点读 learn.EmitForcePlanMetadata 7 keys: force_plan/ratio/alpha/beta/reason/computed_at/session_id, plan.ReadForcePlanHint tolerant parse)** | **D7-S5-A70** | **`plan/force_plan_injection.go` + `mups/learn/force_plan_integration_test.go` (2 tests)** | **IMPLEMENTED (DM-20260707-001 PR-F)** | **P0** | Plan_Field |
...
| **D7-S6-A60-T01** | **`DecisionKind` 5 枚举 (accept / retry / child_worker / parent_rollup / human_review) + String/Marshal/Unmarshal/Validate** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go` + `decision_node_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T02** | **`Decide` interface (input: Verdict + Metadata, output: DecisionKind + Reason + MapRow + JSON)** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T03** | **11 行静态映射表 10 Verdict-based + 1 plan_error catch-all (Pass→accept / Fail→retry / Indeterminate→human_review / Tombstone→parent_rollup / etc.)** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T04** | **`ChildWorkItemSpec` 4 字段 (child_kind / child_input / max_retries / parent_evidence_id) + Validate** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T05** | **`MarshalDecisionJSON` wire format (DecisionKind/Reason/MapRow/JSON 4 字段 round-trip)** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T06** | **`round.DecisionKind/Reason/MapRow/JSON` 4 字段持久化 + ItemPipelineRunner.runDecisionStage 接线 (Stage-5 节点)** | **D7-S6-A60** | **`sessionorchestrator/item_pipeline.go::runDecisionStage`** | **IMPLEMENTED (DM-20260707-001 PR-B)** | **P0** | Decision |
| **D7-S6-A60-T07** | **Decision 表 11 行 plan_error catch-all (ActionUnset/ActionRetry/ActionDecompose/ActionForcePlan/ActionAbort 5 兜底)** | **D7-S6-A60** | **`sessionorchestrator/decision_node.go`** | **IMPLEMENTED (DM-20260707-001 PR-B + PR-F)** | **P0** | Decision |
| **D7-S6-A60-T08** | **Decision 28 单元测试覆盖 11 行映射 + ChildWorkItemSpec 6 子类 + MarshalDecisionJSON round-trip + no-spec omit + map-miss fallback** | **D7-S6-A60** | **`sessionorchestrator/decision_node_test.go` (28 tests)** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T09** | **Negative guard: ChildWorkItemSpec empty child_kind → DecisionKind=human_review, no spawn** | **D7-S6-A60** | **`sessionorchestrator/decision_node_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T10** | **Runaway AttemptNo guard: AttemptNo > MaxAttempts → human_review** | **D7-S6-A60** | **`sessionorchestrator/decision_node_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T11** | **E2E LP-3: parent_rollup sibling gating — Verify 4 子任务 sibling 全部 complete 前不能触发 parent_rollup** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T12** | **E2E LP-3: accept happy path — Pass verdict → DecisionKind=accept, round.Metadata["decision"] 写入** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T13** | **E2E LP-3: DAG fork suppressed when flag off — `devrix.d7.dag_executor.enabled=false` 走 legacy 路径** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T14** | **E2E LP-4: DAG fork suppressed when flag on + nil executor — 不会 panic, 走 human_review** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T15** | **E2E LP-4: fail + max retry → human_review (AttemptNo > MaxAttempts → DecisionKind=human_review)** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T16** | **E2E LP-4: accept preserves base exit reason — DecisionKind=accept 不覆盖 round.ExitReason** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A60-T17** | **E2E LP-4: helper uses production learner — DecisionNode.Decide 不依赖 mock, 用 production learner 路径** | **D7-S6-A60** | **`tests/integration/d7/d7_decision_e2e_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-D)** | **P0** | Decision |
| **D7-S6-A70-T01** | **`LearnPolicy.classifyScenario` 22-scenario 29 sub-test PASS (trivial_qa / single_tool / multi_tool / multi_intent / rollback / etc.)** | **D7-S6-A70** | **`mups/learn/policy.go` + `mups/learn/policy_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
| **D7-S6-A70-T02** | **`AsyncLearner` chan queue + worker=2 (异步触发不阻塞 channel close, Enqueue < 1ms)** | **D7-S6-A70** | **`mups/learn/async.go` + 测试** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
| **D7-S6-A70-T03** | **`EmitForcePlanMetadata` 7 keys contract (force_plan/ratio/alpha/beta/reason/computed_at/session_id) 跨包引用 plan.ReadForcePlanHint** | **D7-S6-A70** | **`mups/learn/force_plan.go` + `mups/learn/force_plan_integration_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
| **D7-S6-A70-T04** | **3-tier degradation (LearnError → re-queue → drop with slog.Warn, 3 次失败降级到 skip)** | **D7-S6-A70** | **`mups/learn/async.go`** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
| **D7-S6-A70-T05** | **`feishu_user_action` listener (D1 IM 透传 user_accept/user_cancel → LearnFeedbackChain)** | **D7-S6-A70** | **`mups/learn/user_action.go` + 测试** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
| **D7-S6-A70-T06** | **11 unit tests 全 PASS (LearnPolicy/AsyncLearner/force_plan/3-tier degradation/UserAction listener)** | **D7-S6-A70** | **`mups/learn/policy_test.go` (5) + `async_test.go` (3) + `user_action_test.go` (3)** | **IMPLEMENTED (DM-20260707-001 PR-E)** | **P0** | Learn |
...
### D7-S15-A102: spawn_decision_algebra kernel + SpawnPolicyEvaluator 3 子决策代数化 (M5, DM-20260705-006)

> **d7-spawn-decision-algebra (DM-20260705-006) — MUPS 5 节点重构总图 M5 落地 (M1+M2+M4 已 S7_archived).**

| T ID | 描述 | 归属 A/F | Test 位置 | Status | Priority | Span Evidence |
|------|------|----------|-----------|--------|----------| --- |
| **D7-S15-A102-T01** | **`checkBudget(round, ctx) (SpawnPolicy, bool)` 7 case (R0/R0.5/R1 w/ cont/R1 w/ exhaust/R1 no schema/R2/fall-through) + 单测 fire true/false 覆盖**: R0 `ctx.RunningChildren > 0` → `SpawnAwait, true`; R0.5 `applicableDeliverableSchema + !deliverableContinuationRequired` → `SpawnNone, true`; R1 w/ cont `ctx.Depth >= MaxDepth + deliverableContinuationRequired` → `spawnForDeliverableContinuation, true`; R1 w/ exhaust `ctx.Depth >= MaxDepth + deliverableInlineWouldExhaust` → `SpawnEscalateHuman, true`; R1 no schema `ctx.Depth >= MaxDepth + no schema` → `SpawnInline, true`; R2 `ctx.DailyLimitExceeded` → `SpawnEscalateHuman, true`; fall-through → `SpawnNone, false` (继续下个子决策) | **D7-S15-A102** | **`workmodel/spawn_decision_algebra_test.go::TestCheckBudget_R0/R05/R1_WithContinuation/R1_InlineExhaustedEscalates/R1_NoDeliverableSchema/R2_DailyLimit/FallThrough` 7/7 PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T02** | **`checkRollupGuard(round, ctx) (SpawnPolicy, bool)` 4 case (at-limit escalate/below-limit inline/non-rollup fall-through/Pass+Rollup skip) + 单测**: RH-MUPS-03 (DM-20260701-001) 跨 verdict 单一权威位置;`ctx.RollupRound=false` → `SpawnNone, false` (非 rollup 轮);`ctx.RollupRound=true + VerdictPass` → `SpawnNone, false` (R3/R4 排除,保持 Pass→None 不变);`ctx.RollupRound=true + Retries>=Max` → `SpawnEscalateHuman, true`;`ctx.RollupRound=true + Retries<Max` → `SpawnInline, true` | **D7-S15-A102** | **`workmodel/spawn_decision_algebra_test.go::TestCheckRollupGuard_AtLimitEscalates/BelowLimitInlines/NonRollupFallThrough/PassSkipsGuard` 4/4 PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T03** | **`checkVerdictDirection(round, ctx) SpawnPolicy` 6 case (R3 Pass+R4 Pass exploration+Pass w/ cont CC-1/R5 Partial/R6 Fail/R7 Indeterminate/R8 unknown) + 单测**: switch on `round.VerdictKind`; 5 case + default R8 覆盖所有 VerdictKind; rollup guard 抽走后 switch 块内不嵌 if-else,逻辑清晰; `VerdictKind(99)` unknown → `SpawnNone` (default catch-all) | **D7-S15-A102** | **`workmodel/spawn_decision_algebra_test.go::TestCheckVerdictDirection_R3R4_CommitmentPass/R3_PassWithContinuation/R5_PartialLowUncertainty/R6_CommitmentFail/R7_IndeterminateRetry/R8_UnknownVerdict` 6/6 PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T04** | **`normalizeCtx(ctx TreeEvalContext) TreeEvalContext` 5 字段 default 兜底 helper (MaxDepth/Threshold/MaxIndeterminateRetries/MaxRollupRetries/MaxInlineRetriesAtMaxDepth) + 单测 default values + preserves non-zero**: TreeEvalContext value type, 返回新 ctx 零成本 copy 不改入参; 抽离 5 行 default 兜底使 SpawnPolicyEvaluator 主函数体 50+→8 行 | **D7-S15-A102** | **`workmodel/spawn_decision_algebra_test.go::TestNormalizeCtx_DefaultValues + TestNormalizeCtx_PreservesNonZero` 2/2 PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T05** | **`SpawnPolicyEvaluator` 22 组合字节级等价旧实现 (build tag `legacy_spawn`)** + 5 rollup 组合 = 27 sub-cases 字节级 PASS: nil round + R0 + 2 R0.5 + 3 R1 + R2 + R3 + R4 + R3 w/ cont + 5 R5 + 4 R6 + 1 R7 retry + R8 unknown + 4 rollup guard (at-limit Partial/Fail/Indeterminate + Pass+Rollup fallthrough). 旧 `SpawnPolicyEvaluatorLegacy` 50+ 行实现保留在 `spawn_policy_legacy_test.go` (build tag `legacy_spawn`), 仅在 `-tags legacy_spawn` 时编译; 下个 change (`mups-cleanup-legacy`) 删除 | **D7-S15-A102** | **`workmodel/spawn_policy_legacy_test.go::TestSpawnPolicyEvaluatorRefactor_ByteEquivalent_OldVsNew` (build tag `legacy_spawn`) 27/27 sub-cases PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T06** | **现有 22 测试 0 修改 PASS (21 `spawn_policy_test.go` + 1 `spawn_policy_inline_test.go`)**: 0 行为变化承诺 1:1 验证; 含 R0/R0.5/R1/R2/R3/R4/R5/R6/R7/R8 + 4 rollup guard (T01-T05 D7-S15-A89) + 6 CC-U1 + ValidateSpawnDecompose + CapChildSpecs + ResolveHint | **D7-S15-A102** | **`workmodel/{spawn_policy,spawn_policy_inline}_test.go` 0 修改** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
| **D7-S15-A102-T07** | **3 子决策顺序锁定测试 (4 sub-cases)**: R0 fired (RunningChildren>0) → checkBudget wins, 后续不被调; rollup guard below-limit → checkRollupGuard wins; Pass+CommitmentPlan+no cont → checkVerdictDirection wins; Pass+Rollup fall-through (D7-S15-A89-T05) → checkVerdictDirection wins (Pass skips rollup guard). 子决策顺序是隐性契约, 显式声明在 SpawnPolicyEvaluator 3 步调用 | **D7-S15-A102** | **`workmodel/spawn_decision_algebra_test.go::TestSpawnPolicyEvaluator_SubDecisionOrder` 4/4 sub-cases PASS** | **IMPLEMENTED (DM-20260705-006)** | **P0** | Spawn_Algebra |
...
| **D7-S2-A50-T11** | **`communication.feishu.EmitPartial(ctx, sessionID, segmentID, content, IdempotencyKey)` 飞书 API streaming 增量发射 (按 segment_id 单调递增 content 长度)** | **D7-S2-A50 RunTurnLoop** | **`internal/layers/communication/feishu/streaming.go::EmitPartial` + `streaming_test.go`** | **IMPLEMENTED (DM-20260707-001 PR-C)** | **P0** |
| **D7-S2-A50-T12** | **`EmitPartial`/`EmitFinal` in-memory dedup table by (session_id, segment_id) — 同一 (session, segment) 多次调用只发一次飞书 API** | **D7-S2-A50 RunTurnLoop** | **`internal/layers/communication/feishu/streaming.go` (dedupMap sync.Mutex) + `streaming_test.go::TestEmitPartial_DedupBySegmentID`** | **IMPLEMENTED (DM-20260707-001 PR-C)** | **P0** |
...
| **4.30.0** | **2026-07-07** | **devrix-d7-multi-intent-observation-decompose (DM-20260707-001) PR-E+PR-F MERGED (step 6/7 + 7/7)**: (1) **PR-E Learn Node 升格** D7-S6-A70-T01..T06 (LearnPolicy + classifyScenario 22-scenario/29 sub-test PASS + AsyncLearner chan queue worker=2 + force_plan link + 3-tier degradation + feishu_user_action listener + 11 unit tests 全 PASS); (2) **PR-F Plan 26 场景** D7-S5-A70-T01..T07 (PlanFieldValidator 10 字段级子类拒绝 + PlanParseReject 6 扩展子类合并 12 + RetryWithFeedback ≤2 retries + DecomposeIntoChildren fallback + PlanErrorDecision abort+NO_Learn + Decision 10→11 行 plan_error 映射 + force_plan Plan injection + 26 scenarios/27 tests PASS) + 跨包 contract test (learn.EmitForcePlanMetadata ↔ plan.ReadForcePlanHint 7 keys 一致). **0 函数签名变化** (pure additive) + 26/26 orchestration packages go test -race 100% PASS + go vet 0 warning. Total 332→339, +7 T PLANNED→IMPLEMENTED, P0 286→293. 详见 `openspec/changes/devrix-d7-multi-intent-observation-decompose/` + PR #457 (PR-E) + PR-F. **变更完结**: DM-20260707-001 7/7 PR 全部 MERGED, 59/59 P0 T 100% IMPLEMENTED, 进度 100%. |
...
| T ID | 描述 | 归属 A/F | Test 位置 | Status | Priority | Span Evidence |
|------|------|----------|-----------|--------|----------| --- |
| **D7-S16-A103-T01** | `interfaces/resolution_contract.go` 5 类型 (ResolutionStrategy/SubWorktreeSpec/ResolutionClaim/ResolutionReport/UnresolvedObs) + 4 验证方法 (NewResolutionStrategy/WithSubWorktree/NewResolutionReport/Validate) | D7-S16-A103 | `internal/layers/orchestration/interfaces/resolution_contract_test.go` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A104-T01** | Plan artifact schema 扩展 `ResolutionStrategy[]` + `sub_worktree` 可选字段 + JSON unmarshal 路径 | D7-S16-A104 | `mups/plan/plan_test.go::TestPlanArtifact_IncludesResolutionStrategies` | **IMPLEMENTED** | **P0** | `D7_Mups_Resolution_Strategy` (Plan Span) |
| **D7-S16-A104-T02** | Plan LLM 引导词 append（fieldMap guide 方式，走 i18n.ResolutionStrategyGuide 而非 prose 块） | D7-S16-A104 | `i18n/format_hints_mups_test.go::TestResolutionStrategyGuide_ZH` + `TestResolutionStrategyGuide_EN` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A104-T03** | `StrategicPlanProposer` 解析 `sub_worktree` 字段；保留 `child_specs[]` 兼容路径（标 deprecated） | D7-S16-A104 | `sessionorchestrator/strategic_plan_proposer_test.go::TestStrategicPlanProposer_ParsesSubWorktree` + `TestLegacyChildSpecsCompat_DeprecationWarning` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A105-T01** | Execute artifact schema 扩展 `ResolutionClaim[]` + JSON unmarshal 路径 + metadata 编码 | D7-S16-A105 | `sessionorchestrator/item_pipeline_test.go::TestBuildArtifactFromWorkItemResult_StampsResolutionClaims` + `TestExtractResolutionClaimsFromArtifact_RoundTrip` | **IMPLEMENTED** | **P0** | `D7_Mups_Resolution_Claim` (Execute Span) |
| **D7-S16-A105-T02** | Execute LLM 引导词 append（tool call 完成后引导 claim，fieldMap guide 方式） | D7-S16-A105 | `sessionorchestrator/resolution_claim_roundtrip_test.go::TestAppendResolutionClaimHint_*` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A105-T03** | Execute LLM 不填 `ResolutionClaim[]` 路径覆盖 (CoverageRatio=0 fallback) | D7-S16-A105 | `sessionorchestrator/item_pipeline_test.go::TestExtractResolutionClaimsFromArtifact_NoArtifact` + `TestExtractResolutionClaimsFromArtifact_MalformedPayload` | **IMPLEMENTED** | **P1** | — |
| **D7-S16-A105-T04** | safety net: Plan 缺 strategy 时退化 detectUserGate + warning log | D7-S16-A105 | `executionflow/verify/resolution_coverage_test.go::TestComputeResolutionCoverage_EmptyStrategiesReturnsNil` | **IMPLEMENTED** | **P1** | — |
| **D7-S16-A105-T05** | safety net: 旧 `execution_mode + child_specs[]` 退化路径 (legacy R0-R8 logic 保留) | D7-S16-A105 | `workmodel/spawn_decide_resolution_test.go::TestCheckResolutionReport_EmptyReport_FallThrough` + `TestCheckResolutionReport_RC4c_LowStrength_NoSubWorktree` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A106-T01** | `verifyResolutionCoverage()` 纯函数 + 4 状态决策表 (17 组合 byte-equivalent) | D7-S16-A106 | `executionflow/verify/resolution_coverage_test.go::TestComputeResolutionCoverage_FourStateMatrix` | **IMPLEMENTED** | **P0** | `D7_Resolution_Coverage` (Verify Span) |
| **D7-S16-A106-T02** | Verify 在 deliverable-verify 之前调用 `verifyResolutionCoverage()` + 输出 ResolutionReport 到 round | D7-S16-A106 | `sessionorchestrator/item_pipeline_test.go::TestExtractResolutionClaimsFromArtifact_RoundTrip` | **IMPLEMENTED** | **P0** | — |
| **D7-S16-A106-T03** | `WorkItemPipelineRound.ResolutionReport` 字段 + round 状态注入 | D7-S16-A106 | `sessionorchestrator/item_pipeline.go::round.ResolutionReport = verify.ComputeResolutionCoverage(...)` | **IMPLEMENTED** | **P0** | — |
| **D7-S5-A108-T01** | `SpawnDecomposeForUnresolved` 分支 + HasSubWorktree 触发逻辑 (治本断链 B P0) | D7-S5-A108 | `workmodel/spawn_decide_resolution_test.go::TestCheckResolutionReport_RC4a_AnySubWorktreePending` + `TestSpawnPolicyEvaluator_RC4a_OverridesPassNone` | **IMPLEMENTED** | **P0** | `D7_Spawn_From_Resolution` (Decide Span) |
| **D7-S5-A108-T02** | `SpawnUserGate` 分支 + 触发条件 (全 HasSubWorktree=false + MaxUnresolvedStrength ≥ 0.85) | D7-S5-A108 | `workmodel/spawn_decide_resolution_test.go::TestCheckResolutionReport_RC4b_HighStrength_NoSubWorktree` + `TestSpawnPolicyEvaluator_RC4b_OverridesVerdictDirection` | **IMPLEMENTED** | **P0** | — |
| **D7-S5-A108-T03** | `SpawnInline` 兜底分支 (RC-4c 沿用现有 R0-R8 逻辑) | D7-S5-A108 | `workmodel/spawn_decide_resolution_test.go::TestCheckResolutionReport_RC4c_LowStrength_NoSubWorktree` | **IMPLEMENTED** | **P1** | — |
| **D7-S5-A108-T04** | tool_filter whitelist = `[ask_user_question]` 注入 (SpawnUserGate 路径) | D7-S5-A108 | `workmodel/spawn_apply.go::DefaultUserGateToolFilter` + `createUserGateWorkItem` + `internal/lint/layer/d7_child_specs_deprecation_test.go` | **IMPLEMENTED** | **P0** | — |
| **D7-S15-A109-T01** | `DecomposeFromSubWorktree` 入口 (替代 `DecomposeChildren` 处理 sub_worktree 路径) | D7-S15-A109 | `workmodel/spawn_budget_gate_test.go::TestBuildChildSpecsFromSubWorktrees_FullSpec` + `composeSubWorktreeDirective` | **IMPLEMENTED** | **P0** | — |
| **D7-S15-A109-T02** | budget gate (depth/children/daily 超限退化 SpawnInline + warning log) | D7-S15-A109 | `workmodel/spawn_budget_gate_test.go::TestApplySpawnPolicy_RC4a_DepthGateDegradesToInline` + `TestApplySpawnPolicy_RC4a_TooManyChildrenDegradesToInline` | **IMPLEMENTED** | **P0** | — |
| **D7-S15-A110-T01** | `WorkItemPipelineRound.ResolutionReport` 字段 + 跨节点注入 (Plan → Verify → Decide 数据流) | D7-S15-A110 | `workmodel/pipeline_round.go::ResolutionReport` + `interfaces/resolution_contract.go::SubWorktree` (Phase 4 wire extension) | **IMPLEMENTED** | **P0** | — |

**D7-RT Total:** 19 T (11 P0 + 8 P1) — **19/19 IMPLEMENTED · 0/19 PLANNED**
...
| **L5-MUPS-FD-6** (DM-20260706-001 devrix-d7-frame-delta-phase1-2-span-trigger) | testutil Phase 1+2+3 span 触发链：SequenceLLMStub.FrameDeltaInject callback + SeedPriorExecContext helper + 5-cycle e2e baseline + AC4 testutil-only 文档化 | D7-S9-A113 | `tests/testutil/d7_frame_delta_helpers_test.go::TestSequenceLLMStub_FrameDeltaInjectCallback` + `tests/integration/d7/d7_frame_delta_e2e_test.go::TestIntegration_D7FrameDelta_Phase1And2SpanTrigger` | **IMPLEMENTED (PR #466)** | **P0** | `D7_Execute_PlanFrameDelta_Inject` + `D7_Observe_PriorDelta_Inject` + `D7_Execute_ConvergenceMetric_Emit` |
| **L5-MUPS-FD-7** (DM-20260706-004 devrix-d7-frame-delta-phase2-production-wiring) | production Phase 2 wiring：observation_proposer.go:257 nil → prevExecCtx upstream (itemObserve + mergeProposedObservations 函数签名 +1 参数) + Round 2 unit test + e2e baseline ≥2 | D7-S5-A111 | `sessionorchestrator/observation_proposer_test.go::TestMergeProposedObservations_NonFirstRoundBuildsPriorDelta` + `tests/integration/d7/d7_frame_delta_e2e_test.go::TestIntegration_D7FrameDelta_Phase1And2SpanTrigger` | **IMPLEMENTED (PR #467)** | **P0** | `D7_Observe_PriorDelta_Inject` |
| **T19** | S3-Gate 三方博弈论 review (codex + cursor + claude 外部评审) | D7-S13-A42 | cc-connect relay (codex + cursor + claude) | **FOLLOW-UP** | **P0** | — |

**D7-FD Total:** 21 T (19 P0 + 2 P1) — **20/21 IMPLEMENTED**（Phase 1-3 16 T via PR #434 + Phase 4 e2e trace replay 3 T via PR #437 + L5-MUPS-FD-6 testutil span trigger via PR #466 + L5-MUPS-FD-7 production wiring via PR #467；span 属性 key 对齐 hardening L648 `convergence.uncertainty_reduction_rate`；5 子测试 PASS = T15 + T16 + T17 + L5-MUPS-FD-6 + L5-MUPS-FD-7）。**1 FOLLOW-UP:** T19 三方 review (codex + cursor + claude 外部评审) — 单独 OpenSpec 入口。
