...
> 14 S → 6 S 后 span operation 重新归类到 6 个 S + 横切 Hardening。共 **26 ops**（18 旧 + **5 新 P0/P1** + **3 内层 observability span**，DM-20260626-009 follow-up）+ 9 sessionSpan attributes。

### D7-S1 WorkModel Operations（State Authority，3 ops）
...
| `D7_Worktree_Op` ⭐新 P1（DM-20260626-009 follow-up 内层 span）| INTERNAL | workmodel | 4.1.0 | session_id, worktree.op, worktree.item_id, worktree.phase_or_status |

### D7-S1 Sub-Worktree Operations（subworktree.run P2，DM-20260626-009 follow-up 内层 span）

| Operation | Kind | Component | Since | Key Attributes |
|-----------|------|-----------|-------|----------------|
| `D7_SubWorktree_Run` ⭐新 P2 | INTERNAL | sessionorchestrator | 4.1.0 | session_id, subworktree.parent_id, subworktree.child_id, subworktree.spawned_by |

> `subworktree.child_id` 当前为空 — `RunParallelExplore` 是 no-op stub，span 形态先就位等 WorkItemExecutor-driven successor 落地后填实。
...
### D7-S5 DecisionPlanning + Observe Operations（Information Producer + Quantizer，5 ops）
...
| `D7_SubTurn_Iteration` ⭐新 P1（DM-20260626-009 follow-up 内层 span）| INTERNAL | sessionorchestrator | 4.1.0 | session_id, subturn.item_id, subturn.iter, subturn.finish_reason, subturn.stop_reason |

> `D7_SubTurn_Iteration` 由 `DefaultWorkItemExecutor.ExecuteWorkItem` 在每个 ReAct iter 结束时触发；cap-hit 时再发 1 个 `iter=max+1 / stop_reason=max_iters` span 让"超出 iter 上限"终止态在 Jaeger 显形。`subturn.finish_reason` 取 LLM 的真实 finish_reason（stop/tool_calls/length/...），与 executor 自定义的 stop_reason（final_answer/max_iters/tool_error/...）正交。
...
| Operation | Kind | Component | Since | Key Attributes |
|-----------|------|-----------|-------|----------------|
| `D7_Interfaces_Task_Spec_Created` ⭐新 P0 | INTERNAL | interfaces | 4.3.0 | session_id, trace_id, plan_kind, channel_kind, workitem_kind, has_blockage |
| `D7_Interfaces_Task_Report_Created` ⭐新 P0 | INTERNAL | interfaces | 4.3.0 | session_id, trace_id, verdict_kind, exit_reason, has_dissent, dissent_count, resource_tokens |
| `D7_TaskReport_Dissent_Recorded` | INTERNAL | interfaces | 4.3.0 | session_id, summary_hash, dissent_count, top_n_truncated |
| `D7_TaskReport_Blockage_Recorded` | INTERNAL | interfaces | 4.3.0 | session_id, blockage_kind, severity, retryable, cause |
| `D7_TaskReport_Resource_Recorded` | INTERNAL | interfaces | 4.3.0 | session_id, token_used, elapsed_ms, step_count |
...
### WorkItem Inner Layer Trace 树（v4.1.0，DM-20260626-009 follow-up）

> 3 个内层 span 补 MUPS 5 节点 root span 与单行 op 之间的可见性 gap。挂在 `D7_Orchestration_Channel_Route`（Execute 节点）下，因为 `ItemPipelineRunner.Run` 是 Execute 阶段的执行路径。

```text
D7_Orchestration_Session_Process
└── D7_Orchestration_Channel_Route  (Execute 节点)
    └── D7_Orchestration_TaskGraph_Synthesize  (Plan 节点 outcome)
        └── D7_Orchestration_Verify_Verdict
            └── D7_Worktree_Op  {op=set_round_phase, phase=observe}  ← ItemPipelineRunner.Run 入口
                └── D7_Worktree_Op  {op=set_round_phase, phase=plan}
                    └── D7_Worktree_Op  {op=set_round_phase, phase=execute}
                        └── D7_SubTurn_Iteration  {iter=1, finish_reason=tool_calls, stop_reason=ok}  ← ReAct iter 1
                            └── D7_SubTurn_Iteration  {iter=2, finish_reason=stop, stop_reason=final_answer}  ← ReAct iter 2 (终止)
                        └── D7_Worktree_Op  {op=set_round_phase, phase=verify}
                        └── D7_Worktree_Op  {op=set_round_phase, phase=learn}
                        └── D7_Worktree_Op  {op=apply_pipeline_round, phase=await_child}
                        └── D7_Worktree_Op  {op=update_status, phase_or_status=completed}
            └── D7_SubWorktree_Run  {parent_id=item_1, child_id=, spawned_by=spawn_parallel_explore}  ← session_turn_loop.go:143，目前 no-op
```

> **注意**：`D7_SubWorktree_Run` 当前 `child_id` 为空字符串，因为 `RunParallelExplore` 是 no-op stub（DM-20260626-009）。span 形态先就位，等 WorkItemExecutor-driven successor 落地后 `child_id` 会被填实。
