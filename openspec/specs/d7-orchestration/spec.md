...
**Last Updated:** 2026-06-30 (devrix-d7-observe-unified-llm-path DM-20260630-001: Observe 退役 A74 独立 D3 LLM 路径; ADDED D7-S16-A75 Unified LLM Entry @ Execute — 首次 LLM 仅走 WorkItemExecutor D2+D3; bootstrap ObservationProposer nil; llm_observation_proposer.go 删除; 对齐 Clawcode QueryLoop)
...
| **D7-S16** | **Layer SubContext (Phase 1+2+3)** | **Per-Layer SubContext + ChildDownlink + cohort CG2′ + Upstream BlockedBy + PeerStatus + ScopeContract + SpawnPolicy gate + Observe R-OBS mapping (no WorkItem private) + ~~LLM ObservationProposer~~ **Unified LLM @ Execute (A75)** + Wave ContextResolver→MaterializePolicy (Phase 3) + `depth≥1` 默认 Materialize (无 feature flag)** | **IMPLEMENTED (Phase 1+2+3 + DM-20260630-001 A75)** | `orchestration/wavescheduler/{context,context_materialize}.go` + `orchestration/sessionorchestrator/{item_observe,observation_proposer,workitem_executor}.go` + `wire_wave.go` 等 |
...
### Requirement: D7-S16-A74 LLM ObservationProposer @ Observe (DM-20260628-002) — SUPERSEDED

> **SUPERSEDED (DM-20260630-001):** T35 裸 D3 + 英文写死 prompt 已废弃。Observe LLM 提案改由 **D7-S16-A75** 约束：必须先 `ContextPreparer.Prepare`（D2）再 D3。

### Requirement: D7-S16-A75 Observe LLM via D2 then D3 (DM-20260630-001)

When `ObservationProposer` is wired (production default: `LLMObservationProposer`), Observe MAY call D3 to propose Obs* candidates. **Every Observe D3 call MUST be preceded by D2 context assembly** via `ContextPreparer.Prepare` (Jaeger `D2_Context_Process`). Direct D3 calls with standalone English prompts are forbidden.

- System prompt = D2 prepared prompt + locale-aware observation task appendix (`zh-CN` default)
- User message = structured signals only (directive, ScopeContract, inbound lines, prior) — **no** WorkItem private ReAct transcript
- `ValidateObservationProposals` rule-gates output (ObsFact strength ≤ 0.85, mandatory evidence)
- LLM/proposer errors are fail-safe (rules-only Observe continues)
- Execute phase retains its own D2→D3 path (`WorkItemExecutor`); Observe and Execute each may emit separate D2+D3 spans in one MUPS round

#### Scenario: Observe D3 follows D2 Prepare
- GIVEN production `LLMObservationProposer` wired with `ContextPreparer`
- WHEN Observe calls `ProposeObservations`
- THEN `ContextPreparer.Prepare` runs before `LLMInvoker.InvokeStream`
- AND Jaeger shows `D2_Context_Process` before Observe-phase `D3_LLM_Stream`
- AND system prompt contains D2 base template plus localized observation appendix

#### Scenario: Observe system prompt uses zh-CN by default
- GIVEN `PromptLocale` is `zh-CN`
- WHEN Observe LLM is invoked
- THEN observation task appendix is Chinese (not the legacy English-only T35 prompt)

#### Scenario: LLM failure fail-safe
- GIVEN `LLMObservationProposer` returns error or empty proposals
- WHEN `observeWorkItem` completes
- THEN R-OBS rule-based observations still populate `UncertaintyReport`

#### Scenario: Test-only StaticObservationProposer
- GIVEN unit test injects `StaticObservationProposer`
- WHEN `mergeProposedObservations` runs
- THEN proposals pass `ValidateObservationProposals` without D2/D3 calls
...
|---------|------|---------|
| **4.19.0** | **2026-06-30** | **devrix-d7-observe-unified-llm-path (DM-20260630-001)**：(1) SUPERSEDED D7-S16-A74（T35 裸 D3 + 英文写死 prompt）；(2) ADDED D7-S16-A75 Observe LLM via D2 then D3（`ContextPreparer.Prepare` → 本地化 Obs 附录 → `LLMInvoker`；禁止裸调 D3）；(3) 重写 `llm_observation_proposer.go` + bootstrap wired；(4) t-registry A75-T01..T04 IMPLEMENTED；A74-T01..T04 SUPERSEDED |
