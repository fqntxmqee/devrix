...
9. **Observe↔LLM I/O 协议（DM-20260708-003 PR #473）**：Observe 节点 frame-level I/O 契约 — 11→6 字段过滤（observeLLMFieldMap 显式 N→M 映射）+ 4 ObsKind JSON schema (ObsFact/ObsSignal/ObsDeviation/ObsUncertainty) + 5 用户场景（4 ObsKind + 1 混合 fact+uncertainty）+ Go-side `validateOneProposal` 兜底（strength cap 0.85 / strength lift 0.5 / evidence 注入 / max=3 / empty reject）+ UncertaintyReport.Partition 路由（CatBusiness / CatSystem+ObsDeviation / CatSystem+ObsUncertainty str≥0.7 → Anomalies）。详见 [`d7-observe-llm-io-protocol-spec.md`](d7-observe-llm-io-protocol-spec.md)
