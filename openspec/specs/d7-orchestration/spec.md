...
**Last Updated:** 2026-07-11 (devrix-d7-observe-node-spec DM-20260711-001: Observe 全节点 spec + P1 fast-path source 过滤; **previous**: devrix-d7-execute-llm-protocol-doc DM-20260708-005: Execute↔ToolRunner 4 Channel 5 场景 I/O 协议沉淀 + 5 trace test; **previous**: devrix-d7-plan-llm-protocol-doc DM-20260708-004 PR #474: Plan↔LLM 5-scenario I/O 协议沉淀 + 5 trace test; **previous**: devrix-d7-observe-llm-protocol-doc DM-20260708-003 PR #473: Observe↔LLM 5-scenario I/O 协议沉淀 + 16 trace test; **previous**: devrix-d7-multi-intent-observation-decompose DM-20260707-001 S7_Archived, 7/7 PR 全部 MERGED (#451 #452 #453 #454 #455 #456 #457 #458): PR-A1 grammar + PR-A2 AC contract + PR-B DAGExecutor + PR-B fix + PR-C streaming + PR-D Decision node + PR-E Learn 22 场景 + PR-F Plan 26 场景; 5 节点管道 → 6 节点 (Observe→Plan→Execute→Verify→**Decision**→Learn); 8 PR 联动 59/59 P0 T 100% IMPLEMENTED 进度 100%; 跨包 force_plan contract (learn ↔ plan 7 keys); 26/26 orchestration packages -race PASS; **previous**: devrix-d7-observational-fastpath DM-20260706-011 2026-07-07: high-strength CatBusiness ObsFact 跳过 Plan+Execute+Verify 直 emit Statement (trivial Q&A 3-5s→~1s, LLM call 3→1); **previous**: devrix-d7-mups-frame-delta-closure 2026-07-05: Phase 1-4 完整链路 FrameDelta + ConvergenceMetric; **previous**: devrix-d7-uncertainty-resolution-traceability 2026-07-05: ResolutionContract + DecideBinding 5-phase S4 闭环)
...
8. **Frame Delta 闭环节流链（S4_Implemented Phase 1-3 — DM-20260705-010 PR #434；Phase 4 trace 重放待 S5）**：Observe→Plan→Execute LLM I/O 帧 delta 显式协议 — `interfaces.FrameDelta` 5 字段（PriorArtifactSummary + KnownGaps + ExecutionMode + ChildSpecs + DeliverableContract）+ Execute `ConvergenceMetric` 3 字段（UncertaintyReductionRate + ObservedGapsClosedCount + FrameDeltaConsumed，deterministic 0 LLM）。append-only 注入，M1/M2 frame 契约 0 修改。详见 [`mups-frame-delta-spec.md`](mups-frame-delta-spec.md) + [change design](../../changes/devrix-d7-mups-frame-delta-closure/design.md)
9. **Observe↔LLM I/O 协议（DM-20260708-003 PR #473；DM-20260711-001 升级）**：Observe 节点 frame-level I/O — 11→6 字段过滤 + 4 ObsKind JSON schema + 证据剖面 OBS-E/U/O（输入不编码 ObsKind）+ Go-side validateOneProposal 兜底 + Partition 路由。详见 [`d7-observe-llm-io-protocol-spec.md`](d7-observe-llm-io-protocol-spec.md)（Review 版）+ 全节点 [`observe-node-spec.md`](observe-node-spec.md)
10. **Plan↔LLM I/O 协议（DM-20260708-004 PR #474）**：Plan 节点 frame-level I/O 契约 — 18 字段 StrategicPlanFrame（pt struct tag 反射 + buildStrategicPlanFrame 显式条件，无 N→M 显式映射；7 data plane + 11 control plane）+ `rawStrategicPlan` 8 字段 JSON wire format + execution_mode 3 选 1 enum（single/decompose/parallel_probe）+ Go-side `validateStrategicPlan` 兜底（enum 校验 / decompose 必填 child_specs / single 必清空 / clamp / cap / budget）+ 5 用户场景（4 PlanKind + 1 混合 single+高 UncertaintyMean+hasHighStrengthFact → applySingleModeUncertaintyGate bypass）+ LLM execution_mode → QuantizedKind → MatchKind → 4 PlanKind 路由。详见 [`d7-plan-llm-io-protocol-spec.md`](d7-plan-llm-io-protocol-spec.md)
11. **Execute↔ToolRunner 4 Channel I/O 协议（DM-20260708-005）**：Execute 节点 4 Channel router 帧级 I/O 契约 — ChannelRequest 3 字段（SessionID/PriorVerdictKinds/*interfaces.TaskSpec 可选）+ ToolRequest 5 字段（SessionID/ToolName/Args/IdempotencyKey/StepID）+ PlanKind 4 enum 1:1 路由 Commit/Protocol/Scenario/Exploration + ArtifactKind 4 + SideEffectStatus 5 + WorkerType 3 矩阵 + sideEffectForScope 映射（PersistTransient→None / Session/Permanent→Committed）+ 7 SentinelError + EXEC_CHANNEL_9001-9007 错误码（RH-D7-09 fix: 9006 timeout / 9007 ctx cancel wire format 不混淆）+ 5 用户场景（4 Channel + 1 混合 timeout+ctx cancel）。详见 [`d7-execute-toolrunner-io-protocol-spec.md`](d7-execute-toolrunner-io-protocol-spec.md)
12. **Observe 全节点协议（DM-20260711-001）**：双轨架构（Go 机械 + LLM 封闭式分类器）+ 输入类型无关证据剖面 OBS-E0–E7 + 用例 OBS-U01–U12 + OBS-O 反查；P1 `pickHighStrengthBusinessFact` source 感知（LLM 优先于 directive echo）。详见 [`observe-node-spec.md`](observe-node-spec.md) + [`d7-observe-llm-io-protocol-spec.md`](d7-observe-llm-io-protocol-spec.md)（Review 版）
