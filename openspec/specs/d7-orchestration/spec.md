...
**Last Updated:** 2026-06-30 (devrix-d7-observe-unified-llm-path DM-20260630-001: SUPERSEDED A74 裸 D3; ADDED A75 Observe LLM via D2 Prepare → 本地化 Obs 附录 → D3; 重写 llm_observation_proposer + bootstrap wired)
...
| **D7-S16** | **Layer SubContext (Phase 1+2+3)** | **Per-Layer SubContext + ChildDownlink + cohort CG2′ + Upstream BlockedBy + PeerStatus + ScopeContract + SpawnPolicy gate + Observe R-OBS mapping (no WorkItem private) + **Observe LLM D2→D3 (A75)** + Wave ContextResolver→MaterializePolicy (Phase 3) + `depth≥1` 默认 Materialize (无 feature flag)** | **IMPLEMENTED (Phase 1+2+3 + DM-20260630-001 A75)** | `orchestration/wavescheduler/{context,context_materialize}.go` + `orchestration/sessionorchestrator/{item_observe,observation_proposer,llm_observation_proposer,workitem_executor}.go` + `wire_wave.go` 等 |
