...
**Last Updated:** 2026-07-05 (d7-observe-closed-classifier-prompt DM-20260705-009: Observe 节点封闭式分类器定位强化 — system_prompt 措辞修复; format_hints_mups.go MODIFIED ZH/EN intro/suffix 强化封闭式分类器 + signal 不足 obs_uncertainty 引导 + directive 不假设完成状态; prompttags_semantics_{zh,en}.go MODIFIED observe.node_role 同步; prompttags_semantics_golden_test.go MODIFIED golden hash 改写锁定; 8 新测试 D2-S15-A99-T01..T05 (4 P0 + 1 P1) + D7-S5-A99-T10..T12 (3 P0) 全 PASS; M1 9 字段契约/i18n guide header/4 alias 解析/prior_parse_reject 反馈链路 0 修改; 0 行为变化; fix/d7-observe-closed-classifier-prompt PR 待开; **previous**: d7-mups-strategy-injection DM-20260705-008: M3 Strategy 抽象注入 WorkItemExecContext — Strategy interface (workmodel 4 method RouteChannel/SpawnOverride/ShouldDecompose/IsReadOnly) + 4 PlanKind 实现 (commitment/protocol/scenario/exploration) + DefaultStrategy registry (1:1 binding + LookupStrategy + RegisterStrategy + init 验证 4 bindings) + WorkItemExecContext.Strategy 字段 (interface, nil 兜底 protocolStrategy) + spawn_decision_algebra checkVerdictDirection 末尾 1 行 SpawnOverride override hook; 4 PlanKind × 5 VerdictKind = 20 组合中 4 行为增量 (commitment+fail/partial 1-step terminal, scenario+fail read-only no retry was SpawnParallelExplore, exploration+pass parallel continue was SpawnNone) + 16 兜底 0 行为变化; CC-1.4 deliverable continuation precedence 通过 SpawnOverride 签名演进 (planKind, verdictKind) → (round) 保留 (4 CC-1.4 测试 0 修改 PASS); 19 NEW 测试 (14 strategy + 5 algebra integration) + 2 UPDATED (R4_ExplorationPass, R6_ScenarioFail M3 行为增量对齐) + 3 UPDATED integration tests (commitmentOnlyPlanner + decomposeAwarePlanner 隔离); 5 节点重构总图 (M1+M2+M3+M4+M5+cleanup) 行为增量最后一步收口; PR #413 MERGED; **previous**: mups-cleanup-legacy DM-20260705-007: 删除 M4 verify_legacy_test.go (307 行) + M5 spawn_policy_legacy_test.go (356 行) + 4 旧符号 + 2 build tag; 0 行为变化; 70 测试 0 修改全 PASS; CI 单元测试 3m15s 绿; PR #411 MERGED)
