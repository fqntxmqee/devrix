...
9. **Observe↔LLM I/O 协议（DM-20260708-003 PR #473）**：Observe 节点 frame-level I/O 契约 — 11→6 字段过滤（observeLLMFieldMap 显式 N→M 映射）+ 4 ObsKind JSON schema (ObsFact/ObsSignal/ObsDeviation/ObsUncertainty) + 5 用户场景（4 ObsKind + 1 混合 fact+uncertainty）+ Go-side `validateOneProposal` 兜底（strength cap 0.85 / strength lift 0.5 / evidence 注入 / max=3 / empty reject）+ UncertaintyReport.Partition 路由（CatBusiness / CatSystem+ObsDeviation / CatSystem+ObsUncertainty str≥0.7 → Anomalies）。详见 [`d7-observe-llm-io-protocol-spec.md`](../../changes/devrix-d7-observe-llm-protocol-doc/specs/d7-orchestration/d7-observe-llm-io-protocol-spec.md)
10. **Plan↔LLM I/O 协议（DM-20260708-004 PR #474）**：Plan 节点 frame-level I/O 契约 — 18 字段 StrategicPlanFrame（pt struct tag 反射 + buildStrategicPlanFrame 显式条件，无 N→M 显式映射；7 data plane + 11 control plane）+ `rawStrategicPlan` 8 字段 JSON wire format + execution_mode 3 选 1 enum（single/decompose/parallel_probe）+ Go-side `validateStrategicPlan` 兜底（enum 校验 / decompose 必填 child_specs / single 必清空 / clamp / cap / budget）+ 5 用户场景（4 PlanKind + 1 混合 single+高 UncertaintyMean+hasHighStrengthFact → applySingleModeUncertaintyGate bypass）+ LLM execution_mode → QuantizedKind → MatchKind → 4 PlanKind 路由。详见 [`d7-plan-llm-io-protocol-spec.md`](../../changes/devrix-d7-plan-llm-protocol-doc/specs/d7-orchestration/d7-plan-llm-io-protocol-spec.md)
