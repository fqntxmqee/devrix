...
| D2-S2-A02 | ExecuteToolRound | A-BE | tool_calls | tool_results | `engine.go` (→ v3.1 `query/tool_round.go`) |
...
| D2-S3-A02 | FilterTools | A-BE | all_tools, mode | visible | `permission_tools.go` |
...
| D2-S4-A04 | CommitWindow | A-BE | session, budget | trimmed | `engine.go` |

### D2-S5: NestedExecution（嵌套子代理执行）

| A ID | Name | Type | Input | Output | Code Location |
|------|------|------|-------|--------|---------------|
| D2-S5-A01 | SpawnSubquery | A-BE | subquery_spec | result | `nested/subquery.go` |
| D2-S5-A02 | RunBackgroundTask | A-BE | task_spec | task_id | `nested/background.go` |
| D2-S5-A03 | MergeSubResult | A-BE | sub_result | merged_context | `nested/subquery.go` |

---

## Legacy: D2 Harness（#deprecated）

> `#deprecated`: 仅 `query_loop.enabled=false` 时激活。v2.0 将移除。
> DM-20260611-004: QueryLoop 为唯一生产路径。

| A ID | Name | Type | Status | Code Location |
|------|------|------|--------|---------------|
| D2-HARNESS-A01 | Bootstrap | A-BE | DEPRECATED | `harness/bootstrap.go` |
| D2-HARNESS-A02 | Preflight | A-BE | DEPRECATED | `harness/preflight.go` |
| D2-HARNESS-A03 | Route | A-BE | DEPRECATED | `harness/router.go` |
| D2-HARNESS-A04 | ManageToolPool | A-BE | DEPRECATED | `harness/toolpool.go` |
