...
|-----------|-------------|------------------------------|
| Turn 前上下文合法、在预算内 | D2-S15 PrepareExecutionContext | **D2_Context_Loading_Compression** |
| LLM↔Tool 多轮 | ~~D2-S16 RunQueryLoop~~ → **D7-S2-A06 RunTurnLoop**（DM-20260618-010 REMOVED） | (归 D7 ValueFlow) |
| Tool 权限/沙箱先于执行 | D2-S18 EnforceExecutionPolicy | **D2_Tool_Permission_Sandbox** |
| Turn 后状态 durable + deferred complete | D2-S17 PersistSessionState | **D2_Session_State_Persistence** |
| 嵌套 SubQuery/Background 有边界 | ~~D2-S19 NestedExecution~~ → S15+S18 拆分 | (历史) |
| ~~Legacy Harness 仅显式配置~~ | D2-S20 LegacyHarnessFallback（**REMOVED v6.5.0**） | (历史) |
...
| FlowEvent / WorkPlan | D7-S4 | ✅ DM-018 slice-c RESOLVED — 实现迁 `internal/layers/orchestration/executionflow/bridge/flow_reporter.go`；D2 消费端口 `contracts.SubQueryFlowReporter`；治理常量 `orchtypes.BoundaryDM018SliceC = "boundary-debt:dm-018-slice-c-v7.0"` |
| 结论质量 / 信誉 | D6 | Judge |
| **跨域 fixture (summarizer/prepared_turn)** | ⚠️ 待定 (v9.0) | D2 根目录 2 个 fixture (`summarizer_fixture.go` + `prepared_turn_fixture.go`) 保留，无 import cycle 风险但语义属 D7 域；治理常量 `orchtypes.BoundaryCrossDomainFixtures = "boundary-debt:cross-domain-fixtures-v9.0"` |

---

## Boundary Debt Decisions (DM-20260629-002 PR-8)

> 跨域归属争议的 D2 能力, 每项分配 boundary-debt ID 便于后续 v9.0 重新评估。
> 治理常量在 `internal/layers/contextengine/orchtypes/boundary_decision.go`。
> 4 单元测试: `internal/layers/contextengine/orchtypes/boundary_decision_test.go`。
