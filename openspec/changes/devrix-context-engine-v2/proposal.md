...
**Based on:** `devrix-context-engine` (V1 archived), `devrix-llm-gateway` (in progress)
...
| `contracts.go` | +`IVerifyCommandRunner` |
...
| 风险 | 缓解 |
|------|------|
| Autocompact 增加 LLM 成本 | 轻量模型（`autocompact.model`）+ summary_max_tokens 上限 |
| Verify 命令安全风险 | executable+args 无 shell + 白名单 + WorkDir 沙箱 + 超时 |
| LLM Gateway 未就绪 | M1 可并行 stub TokenCounter 接口，集成前切换 |
| 摘要幻觉 | 结构化 JSON 输出 + 仅总结已有消息 |
