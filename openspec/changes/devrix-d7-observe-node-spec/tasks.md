...
**Status legend:** PLANNED / IN_PROGRESS / IMPLEMENTED
...
| D7-S5-A122-T03 | `observe_node_protocol` | IMPLEMENTED | change 包 demand/proposal/design/delta spec | `changes/devrix-d7-observe-node-spec/` |
...
| T ID | L4 | Status | Description | Evidence |
|------|-----|--------|-------------|----------|
| D7-S5-A122-T09 | `observe_category_promote` | PLANNED | NEW `observe_category_promote.go` + wiring | `item_observe.go` |
| D7-S5-A122-T10 | `observe_llm_classifier` | PLANNED | LLM frame 省略 `scope_open_question` | `llm_observation_proposer.go` |

## Phase E — P4 signal + P5 SoT（P1，Wave 4）⏳

| T ID | L4 | Status | Description | Evidence |
|------|-----|--------|-------------|----------|
| D7-S5-A122-T11 | `observe_signal_registry` | PLANNED | NEW registry + `buildObserveSignalInput` | `observe_signal_registry.go` |
| D7-S5-A122-T12 | `observe_llm_classifier` | PLANNED | `observeLLMFieldMap` → pt-tag 派生 | `llm_observation_proposer.go` |

## Phase F — 验证与归档（P0）⏳

| T ID | L4 | Status | Description | Evidence |
|------|-----|--------|-------------|----------|
| D7-S5-A122-T13 | — | PLANNED | 全量 orchestration test PASS | CI |
| D7-S5-A122-T14 | — | PLANNED | acceptance-report.md + S5 用户验收 | change 目录 |

---

**Wave 1–2 准出**：T01–T08 IMPLEMENTED → **可进入 S5 验收（P0 范围）**
**Wave 3–4 待办**：T09–T12（P1 实现债）
