...
- AND a "turn" is defined as all messages from one `user` message (inclusive) to the next `user` message (exclusive), including all intermediate assistant/tool_call/tool_result messages

#### Scenario: Autocompact not triggered when turns insufficient

- GIVEN total turn count is ≤ `preserve_head_turns + preserve_tail_turns`
- WHEN autocompact trigger is evaluated
- THEN autocompact is not triggered regardless of token count

#### Scenario: Autocompact LLM failure degrades gracefully

- GIVEN autocompact is enabled
- WHEN LLM call times out (>10s), fails, or returns invalid JSON after 1 retry
...
- AND the failure reason (timeout/network/json_parse) is included in the error message under `CTX_AUTOCOMPACT_4010`
...
系统 MUST 为压缩各步骤、Autocompact 与 Verify 命令发射可观测事件。采用两个独立接口保持单一职责。
...
- THEN `IPEVObserver.EmitVerifyCommand` includes command name, exit code, and duration

#### Scenario: Metrics label cardinality control

- GIVEN verify commands emit metrics
- WHEN each command completes
- THEN the `command` label value is limited to configured `name` values matching `[a-z0-9_-]+`
- AND `exit_code` is NOT used as a label
- AND command failures are reported via `devrix_ctx_verify_command_failures_total` counter with `command` label

#### Scenario: Verify command count limit

- GIVEN `verify_commands` configuration
- WHEN config is loaded
- THEN command count MUST be ≤ 10 or config is rejected
...
### Requirement: PEV Engine (V2 Enhanced)

系统 MUST 实现 PEV 循环：Execute → Verify（无 Plan 阶段）。V1 默认 `verify_mode: basic`；V2 MUST additionally support `verify_mode: commands` 与 `none`。`verifyPEV` 改为 `PEVEngine` 方法，便于访问注入的 `IVerifyCommandRunner`。`IPEVObserver` 独立于 `ICompressionObserver`。
