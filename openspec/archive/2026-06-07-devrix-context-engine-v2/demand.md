...
### Q11: Autocompact 超时上限？（Grill 2026-06-07）

**A**: 10s（原 30s 不可接受）。P99 摘要延迟必须在 10s 内完成，超时降级等同 V1。 — 2026-06-07

### Q12: Observer 接口职责划分？（Grill 2026-06-07）

**A**: 拆为 `ICompressionObserver`（压缩步骤 + Autocompact）+ `IPEVObserver`（Verify Commands；V3 扩展 Plan），保持单一职责。 — 2026-06-07

### Q13: Autocompact 中 "turn" 如何定义？（Grill 2026-06-07）

**A**: 以 `user` role 为边界——从上一条 user 消息（含）到下一条 user 消息（不含）为一个 turn，中间所有 assistant/tool 消息属于该 turn。 — 2026-06-07

### Q14: Verify Commands metrics 标签基数如何控制？（Grill 2026-06-07）

**A**: name regex `[a-z0-9_-]+` + ≤10 上限 + 不加 exit_code 标签；失败用独立 counter `devrix_ctx_verify_command_failures_total`。 — 2026-06-07
