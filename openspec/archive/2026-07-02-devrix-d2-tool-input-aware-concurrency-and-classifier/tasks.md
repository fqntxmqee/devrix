...
- [x] 开新 feature branch `feat/devrix-d2-tool-input-aware-concurrency-and-classifier`
...
## 验证状态 (本地 — 阶段 6 + 6+ 后, S5 验收完成)

- [x] `go build ./...` 0 errors
- [x] `go test -count=1 ./internal/layers/...` 全量 PASS
- [x] `go test -race -count=1 ./...` 全量 PASS (master 预存失败 tools/ci-lint-invariant 除外)
- [x] 13 T 全 IMPLEMENTED + 21 AC 全 PASS (P0 14 + P1 4 + P2 3)
- [x] 50 文件 e2e 并发版 < 串行 / 3 (AC10)
- [x] `partition_invariants_test` 全 PASS (AC15 完整性 / AC16 交错 / AC17 read部分失败 / AC19 panic 隔离 / AC20 限流 / AC21 goleak)
- [x] read_file IsConcurrencySafe 大/小 input 均 true (AC18: 8K 回归锁)
- [x] AutoModeClassifier interface stub panic 信息合规
- [x] ChannelRouter 占位代码不破坏 partition 行为
- [x] 4 tech-debt 关闭 (TD-STE-01/02/03/06) — 走 PR-F 同步标注
- [x] GrowthBook bash threshold flag 单测 PASS (Production-Safety)
...
|----|--------|------|------|----|------------------|
| **PR-A** | `3257e0bb` | T16-T17 ToolSurface v4 + 19 工具 IsConcurrencySafe 默认 (4 override + 15 default) | 2 | AC1-AC2 + **AC18** | TD-STE-06 |
| **PR-B** | `8e61bb13` | T18-T19 partitionToolCalls + `partition_invariants_test` + 50 文件 e2e 并发版 | 2 | AC3 + AC10 + **AC15-AC17 + AC19-AC21** | TD-STE-01 |
| **PR-C** | `dd8736e7` | T20-T21 toCompactBlock + 19 工具 ToAutoClassifierInput 默认 | 2 | AC4-AC6 | — |
| **PR-D+E** | `57469504` | T22'-T23' AutoModeClassifier P2 interface stub + T24' 单测 + T25' GrowthBook bash threshold flag | 4 | AC7-AC11 | — |
| **PR-F** | `cbcc57d9` + `c0ef5954` + `1763b2cb` | T26 Bash sibling abort + T27 Discard on fallback + T28 inputsEquivalent | 3 | AC12-AC14 | TD-STE-02 + TD-STE-03 |
