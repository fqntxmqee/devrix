...
func (s stubAdapter) Protocol() string { return streamadapter.ProtocolStub }

func testGateway(t *testing.T, handler func(model string) (<-chan *llmgateway.AdapterChunk, error)) *stream.Gateway {
	t.Helper()
...
func TestGateway_should_not_open_circuit_on_context_cancel(t *testing.T) {
	...
	cb := protect.New(cfg.CircuitBreaker)
	gw := stream.New(stream.Deps{
		Config: cfg, Registry: reg, Breaker: cb, Retry: protect.NewExecutor(), Counter: counter,
	})
