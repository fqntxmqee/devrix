...
// fakeClock drives the breaker for deterministic open→half-open transitions.
...
func TestBreakerObserver_emits_state_gauge_and_transition_counter(t *testing.T) {
	...
	cb := breaker.New(cfg).
		WithClock(clock.Now).
		WithObserver(gateway.NewBreakerObserver(obs, gateway.PublishBreakerStateDefault{}))
...
func TestBreakerObserver_nil_observer_is_noop(t *testing.T) {
	...
	cb := breaker.New(cfg) // no observer
...
func TestBreakerObserver_publishes_engine_event_on_transition(t *testing.T) {
	...
	cb := breaker.New(cfg).
		WithClock(clock.Now).
		WithObserver(gateway.NewBreakerObserver(obs, pub))
