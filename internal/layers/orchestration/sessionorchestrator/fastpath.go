...
// FastPath is a zero-allocation proxy that forwards a orchtypes.ProcessRequest to
...
func NewFastPath(cfg *orchtypes.Config, executor TurnExecutor, sink EventPublisher) *FastPath {
	if cfg == nil {
...
// the decision.
func (fp *FastPath) Run(ctx context.Context, req orchtypes.ProcessRequest, systemPrompt string) (<-chan *contracts.EngineEvent, error) {
	if fp.executor == nil {
