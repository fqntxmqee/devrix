...
func (s *ExecutorSelector) SelectExecutor(node wave.TaskNode) (*SelectResult, error) {
	if node.ID == "" {
	...
	case wave.WorkerCursor, wave.WorkerClaudeCode:
	...
	if taskType, ok := node.Metadata["task_type"].(string); ok {
		...
		case TaskTypeExplore, TaskTypePlan:
		...
		case TaskTypeExecute, TaskTypeBackground:
...
func (s *ExecutorSelector) MatchExecutorByTaskType(taskType TaskType) ExecutorType {
	switch taskType {
	case TaskTypeExplore, TaskTypePlan:
		return ExecutorD2
	case TaskTypeExecute, TaskTypeBackground:
