...
// from the streaming response and produces valid wave.TaskNodes.
func TestLLMDecomposer_Decompose_HappyPath(t *testing.T) {
	...
	if nodes[0].ID != "design_api" || nodes[0].WorkerType != wave.WorkerCursor ||
		nodes[0].ContextPolicy != wave.ContextFresh || len(nodes[0].DependsOn) != 0 {
...
func TestLLMDecomposer_Decompose_InvalidEnumsCoerced(t *testing.T) {
	...
	if nodes[0].WorkerType != wave.WorkerCursor {
		t.Errorf("WorkerType = %v, want cursor (default)", nodes[0].WorkerType)
	}
	if nodes[0].ContextPolicy != wave.ContextFresh {
