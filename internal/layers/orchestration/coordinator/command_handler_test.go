...
func TestCommandHandler_Handle_PlanCommand(t *testing.T) {
	...
	ch, err := h.Handle(context.Background(), ProcessRequest{
		SessionID: "sess-1",
	...
	// text content should mention PlanMode (the canonical "Plan mode:" prefix).
	var text string
	for _, ev := range events {
		if ev.Type == "text" {
			text = ev.Content
	...
	if !strings.Contains(text, "Plan") {
		t.Fatalf("text should mention Plan, got %q", text)
	}
}
...
func TestCommandHandler_Handle_NilWiring(t *testing.T) {
	...
	_, err := h.Handle(context.Background(), ProcessRequest{
		SessionID: "sess-nil",
		Message:   "/plan",
	}, IntentClassification{Kind: IntentCommand, Command: "/plan"})
	if err == nil {
