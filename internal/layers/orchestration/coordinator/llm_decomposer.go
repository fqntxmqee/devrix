...
// and mapping the result into []wave.TaskNode.
//
...
func (d *LLMDecomposer) Decompose(ctx context.Context, sessionID, goal string) ([]wave.TaskNode, error) {
	if d == nil || d.llm == nil {
...
// parseDecomposedTasks decodes raw JSON into validated wave.TaskNodes.
...
// graph.
func parseDecomposedTasks(raw, sessionID string) ([]wave.TaskNode, error) {
	...
	nodes := make([]wave.TaskNode, 0, len(dtos))
	for i, dto := range dtos {
		...
		workerType := wave.WorkerType(strings.ToLower(strings.TrimSpace(dto.WorkerType)))
		if !workerType.Valid() {
			workerType = wave.WorkerCursor
		}

		contextPolicy := wave.ContextPolicy(strings.ToLower(strings.TrimSpace(dto.ContextPolicy)))
