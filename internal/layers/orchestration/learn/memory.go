// Package learn: Memory 3-channel interface and implementations (PR-E4).
//
// Learn's LP-2 principle requires that LearningAsset classes be partitioned
...
// ErrAssetClassMismatch (fail-fast at the boundary).
//
...
// allowedClasses returns the LearningClass set accepted by each channel.
// Compile-time guarantee that LP-2 partitioning is exhaustive across the
// 5 LearningClass values.
func (c MemoryChannel) allowedClasses() map[LearningClass]bool {
	switch c {
	case MemorySkill:
		return map[LearningClass]bool{
			LearningClass(types.LearningSOP):      true,
			LearningClass(types.LearningProtocol): true,
		}
	case MemoryFeedback:
		return map[LearningClass]bool{
			LearningClass(types.LearningKnowledge):  true,
			LearningClass(types.LearningConclusion): true,
		}
	case MemoryScheduled:
		return map[LearningClass]bool{
			LearningClass(types.LearningPending): true,
		}
	default:
		return nil
	}
...
func NewSkillMemory() *SkillMemory {
	return &SkillMemory{
		store: make(map[string]*LearningAsset),
	}
}
...
func (m *FeedbackMemory) Store(ctx context.Context, asset *LearningAsset) error {
	...
	if !MemoryFeedback.allowedClasses()[asset.Class] {
		return ErrAssetClassMismatch
	}
	m.mu.Lock()
	defer m.mu.Unlock()
...
// Retrieve returns the asset by AssetKey, or (nil, nil) if absent.
func (m *FeedbackMemory) Retrieve(ctx context.Context, key string) (*LearningAsset, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.store[key], nil
}
...
// DefaultScheduledMaxRetries — matches PendingAssetContent validator upper bound.
const DefaultScheduledMaxRetries = 3
...
// DefaultScheduledMaxRetries when ≤ 0.
func (m *ScheduledMemory) Store(ctx context.Context, asset *LearningAsset) error {
	if asset == nil {
		return ErrAssetIncomplete
	}
	if !MemoryScheduled.allowedClasses()[asset.Class] {
		return ErrAssetClassMismatch
	}
	maxRetries := DefaultScheduledMaxRetries
	if pending, ok := asset.Content.(*PendingAssetContent); ok && pending.MaxRetries > 0 {
		maxRetries = pending.MaxRetries
	}
	triggerAt := asset.ExpiryAt
	if pending, ok := asset.Content.(*PendingAssetContent); ok && !pending.NextRetryAt.IsZero() {
...
func filterAssets(store map[string]*LearningAsset, filter MemoryFilter) []*LearningAsset {
	out := make([]*LearningAsset, 0, len(store))
	now := time.Now()
	for _, a := range store {
		if filter.Class != LearningClass(types.LearningUnknown) && a.Class != filter.Class {
			continue
		}
