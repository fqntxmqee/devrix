...
// falls through to orchtypes.IntentOrchestrate). v1.1 may promote it to a
// fallback decision path.
type LLMIntentClassifier interface {
	ClassifyIntent(ctx context.Context, message string) (orchtypes.IntentClassification, error)
}
...
// LLMIntentClassifier (shadow). When the rule returns orchtypes.IntentOrchestrate
...
// orchtypes.IntentOrchestrate AND llm is configured, the LLM is invoked
...
func (s *ShadowClassifier) Classify(ctx context.Context, message string) (orchtypes.IntentClassification, error) {
	if s == nil {
		return orchtypes.IntentClassification{}, errShadowNil
	}
	...
	if result.Reason == "loop_first_default" {
		if rc, ok := s.rule.(*RuleClassifier); ok {
			...
			if legErr != nil || legacy.Kind != orchtypes.IntentOrchestrate {
				return result, nil
			}
	...
	} else if result.Kind != orchtypes.IntentOrchestrate {
		return result, nil
	}
	if s.llm == nil {
		if s.metrics != nil {
...
func (s *ShadowClassifier) shadowAsync(ctx context.Context, message string, ruleResult orchtypes.IntentClassification) {
	defer func() {
		if r := recover(); r != nil {
