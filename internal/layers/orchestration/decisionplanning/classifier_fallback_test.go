...
func TestLLMFallbackClassifier_Classify(t *testing.T) {
	rule := NewRuleClassifier(orchtypes.RuleOrchestrateConfig())
	...
	tests := []struct {
	...
	}{
		{
			...
			minConfidence: 70,
			llmConfidence: 50,
			wantKind:       orchtypes.IntentCommand,
			wantConfidence: 100,
		},
		{
			...
			minConfidence: 70,
			llmConfidence: 85,
			wantKind:       orchtypes.IntentOrchestrate,
			wantConfidence: 85,
		},
		{
...
func (m *mockLLMClassifier) ClassifyIntent(ctx context.Context, message string) (orchtypes.IntentClassification, error) {
	if m.onCall != nil {
		m.onCall()
	}
	if m.err != nil {
		return orchtypes.IntentClassification{}, m.err
	}
...
func (s *slowLLMClassifier) ClassifyIntent(ctx context.Context, message string) (orchtypes.IntentClassification, error) {
	select {
	case <-ctx.Done():
