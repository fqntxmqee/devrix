...
func TestIsParentRollupSynth_falseWhenLeafHasNoChildren(t *testing.T) {
	tm := NewTaskManager()
	sessionID := "sess-leaf-rollup"
	...
	if IsParentRollupSynth(tm, sessionID, got) {
		t.Fatal("leaf explore with NeedsRollup and no children must not be parent rollup")
	}
}

func TestIsParentRollupSynth_trueWhenChildrenExist(t *testing.T) {
	tm := NewTaskManager()
	...
	if !IsParentRollupSynth(tm, sessionID, got) {
		t.Fatal("expected parent rollup when non-ephemeral children exist")
	}
}
