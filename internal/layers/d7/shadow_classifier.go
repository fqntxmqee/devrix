...
func NewShadowMetrics(meter *metrics.Meter) *ShadowMetrics {
	...
	if err != nil {
		slog.Error("d7: failed to register shadow match counter", "err", err)
		return nil
	}
	mismatch, err := meter.Int64Counter("orchestration.intent.classify.shadow.mismatch")
	if err != nil {
		slog.Error("d7: failed to register shadow mismatch counter", "err", err)
		return nil
	}
	errC, err := meter.Int64Counter("orchestration.intent.classify.shadow.error")
	if err != nil {
		slog.Error("d7: failed to register shadow error counter", "err", err)
		return nil
	}
	disabled, err := meter.Int64Counter("orchestration.intent.classify.shadow.disabled")
	if err != nil {
		slog.Error("d7: failed to register shadow disabled counter", "err", err)
		return nil
	}
	...
	if err != nil {
		slog.Error("d7: failed to register shadow latency histogram", "err", err)
		return nil
	}
...
func (s *ShadowClassifier) shadowAsync(ctx context.Context, message string, ruleResult IntentClassification) {
	defer func() {
		if r := recover(); r != nil {
			s.log.Error("d7: shadow classify panic recovered",
	...
	if err != nil {
		...
		s.log.Warn("d7: shadow classify error",
			"err", err,
	...
	s.log.Info("d7: shadow classify mismatch",
		"rule_kind", ruleResult.Kind,
...
var errShadowNil = &shadowError{Message: "d7: ShadowClassifier is nil"}
