	...
	tests := []struct {
		...
		},
		{
			...
			card: core.NewCard().
				Title("操作", "blue").
				Buttons(
					core.PrimaryBtn("确认", "confirm"),
					core.DefaultBtn("取消", "cancel"),
				).
				Build(),
			wantSchema:  "2.0",
			wantHeader:  "操作",
		...
		},
		{
			...
			card: core.NewCard().
				Buttons(core.DangerBtn("删除", "delete")).
				Build(),
			wantSchema:  "2.0",
			wantElement: "action",
		},
		...
		},
		{
			...
			card: core.NewCard().
				Select("Choose", []core.CardSelectOption{
					{Text: "A", Value: "a"},
					{Text: "B", Value: "b"},
				}, "a").
				Build(),
...
func TestFeishuIntegration_CardHasButtons(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "card with select",
			card: core.NewCard().
				Select("choose", []core.CardSelectOption{{Text: "A", Value: "a"}}, "").
				Build(),
			wantButtons: true,
		},
		{
