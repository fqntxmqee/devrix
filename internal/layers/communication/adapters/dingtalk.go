...
// DingTalkAdapter integrates DingTalk chatbot callbacks with the gateway.
...
var _ gateway.EventHandler = (*DingTalkAdapter)(nil)
...
func WithDingTalkGateway(gw gateway.GatewayAPI) DingTalkAdapterOption {
	return func(a *DingTalkAdapter) {
		a.gateway = gw
	}
}
...
func (a *DingTalkAdapter) SetGateway(gw gateway.GatewayAPI) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.gateway = gw
}
