...
type FeishuAdapter struct {
	...
	eventHandler capture.EventHandler
	api          FeishuAPI
...
// Ensure FeishuAdapter implements capture.EventHandler
var _ capture.EventHandler = (*FeishuAdapter)(nil)
...
func WithGateway(gw capture.GatewayAPI) FeishuAdapterOption {
	return func(a *FeishuAdapter) {
		a.gateway = gw
	}
}
...
func (a *FeishuAdapter) SetGateway(gw *capture.CommunicationGateway) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.gateway = gw
}
...
func (a *FeishuAdapter) SetEventHandler(h capture.EventHandler) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.eventHandler = h
}
