...
func (r *taskStopRunner) Schema() ToolSchema {
	return ToolSchema{
		Name:        "task_stop",
...
func (r *taskStopRunner) Execute(ctx context.Context, _, input string) (*ToolResult, error) {
	sessionID := ToolSessionIDFromContext(ctx)
	if sessionID == "" {
		return &ToolResult{Error: "task_stop: session_id unavailable"}, nil
	}
	fields := parseToolInput(input)
	taskID := strings.TrimSpace(fields["task_id"])
	if taskID == "" {
		return &ToolResult{Error: "task_stop: task_id is required"}, nil
	}
	reg := backgroundRegistry()
	if reg == nil {
		return &ToolResult{Error: "task_stop: background registry not initialized"}, nil
	}
	task, ok := reg.Get(taskID)
	if !ok {
		return &ToolResult{Error: fmt.Sprintf("task_stop: unknown task_id %s", taskID)}, nil
	}
	if task.SessionID != sessionID {
		return &ToolResult{Error: "task_stop: task belongs to a different session"}, nil
	}
	prev := task.Status
...
func (r *taskOutputRunner) Schema() ToolSchema {
	return ToolSchema{
		Name:        "task_output",
...
func (r *taskOutputRunner) Execute(ctx context.Context, _, input string) (*ToolResult, error) {
	sessionID := ToolSessionIDFromContext(ctx)
	if sessionID == "" {
		return &ToolResult{Error: "task_output: session_id unavailable"}, nil
	}
	fields := parseToolInput(input)
	taskID := strings.TrimSpace(fields["task_id"])
	if taskID == "" {
		return &ToolResult{Error: "task_output: task_id is required"}, nil
	}
	...
	reg := backgroundRegistry()
	if reg == nil {
		return &ToolResult{Error: "task_output: background registry not initialized"}, nil
	}
	...
	task, ok := reg.Get(taskID)
	if !ok {
		return &ToolResult{Error: fmt.Sprintf("task_output: unknown task_id %s", taskID)}, nil
	}
	if task.SessionID != sessionID {
		return &ToolResult{Error: "task_output: task belongs to a different session"}, nil
	}
...
func (r *taskListBackgroundRunner) Schema() ToolSchema {
	return ToolSchema{
...
func (r *taskListBackgroundRunner) Execute(ctx context.Context, _, _ string) (*ToolResult, error) {
	sessionID := ToolSessionIDFromContext(ctx)
	if sessionID == "" {
		return &ToolResult{Error: "task_list_background: session_id unavailable"}, nil
	}
	reg := backgroundRegistry()
	if reg == nil {
		return &ToolResult{Error: "task_list_background: background registry not initialized"}, nil
	}
