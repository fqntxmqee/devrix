...
func TestSubQuery_should_filter_read_only_tools_and_set_agent_id(t *testing.T) {
	...
	res, err := query.Run(context.Background(), query.LoopDeps{Loop: loop}, query.SubQueryParams{
		ParentSC:       parent,
...
func TestSubQuery_resume_should_load_sidechain_messages(t *testing.T) {
	...
	res, err := query.Run(context.Background(), query.LoopDeps{Loop: loop, Sidechain: store}, query.SubQueryParams{
		ParentSC:       parent,
		AgentID:        "plan_1",
		AgentName:      "Plan",
		SystemPrompt:   "plan",
