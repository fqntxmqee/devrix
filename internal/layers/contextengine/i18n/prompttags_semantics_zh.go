...
var prompttagsSemanticsZH = map[string]string{
	...
	"observe.input.prior_parse_reject.when_use": "上一轮 Observe wholebody parse/budget 失败反馈（control）；按 code/field 修正输出",
	...
	"plan.input.prior_parse_reject.when_use":    "上一轮 Plan JSON parse 或 budget reject（control）；勿重复相同违规",
	...
	"plan.input.work_item_id.when_use":        "工作项 ID (control, 标识符)",
	"plan.input.observation_ids.when_use":      "上一轮 Obs IDs (data, 增量上下文)",
	"plan.input.depth.when_use":               "当前工作项深度 (control)",
	"plan.input.max_depth.when_use":           "允许的最大深度 (control)",
	"plan.input.existing_children.when_use":   "已存在非临时子节点数 (control)",
	"plan.input.max_children.when_use":        "允许的最大子节点数 (control)",
	"plan.input.decompose_used_today.when_use": "24h 内已用 decompose 次数 (control)",
	"plan.input.remaining_daily.when_use":     "今日 decompose 剩余预算 (control)",
	"plan.input.max_daily.when_use":           "每日 decompose 上限 (control)",
	"plan.input.max_iters.when_use":           "单 WorkItem ReAct 迭代上限 (control)",
	"plan.input.parent_scope_in.when_use":     "父级 scope in 路径 (control, 子集约束)",
