...
func (s stubToolLister) ListTools(_ context.Context, _ string) ([]fallback.ToolDesc, error) {
	return s.tools, nil
}
...
func TestBootstrap_should_run_stages_in_order(t *testing.T) {
	...
	lister := stubToolLister{tools: []fallback.ToolDesc{
		{Name: "bash"},
		{Name: "read_file"},
		{Name: "write_file"},
		{Name: "mcp_search"},
	...
	boot := fallback.NewBootstrap(fallback.BootstrapDeps{
...
func TestBootstrap_should_skip_deferred_flags_when_untrusted(t *testing.T) {
	...
	boot := fallback.NewBootstrap(fallback.BootstrapDeps{
		...
		ToolsReg: stubToolLister{tools: []fallback.ToolDesc{{Name: "bash"}}},
	})
