...
func TestToolPoolFilter_should_apply_simple_mode(t *testing.T) {
	all := []fallback.ToolDesc{
		{Name: "bash"},
		{Name: "read_file"},
		{Name: "write_file"},
		{Name: "call_claude-code"},
	}
	filter := fallback.NewToolPoolFilter(config.ToolPoolConfig{SimpleMode: true, IncludeMCP: true})
...
func TestToolPoolFilter_should_exclude_mcp_when_disabled(t *testing.T) {
	...
	filter := fallback.NewToolPoolFilter(config.ToolPoolConfig{IncludeMCP: false})
	out := filter.Filter(all)
