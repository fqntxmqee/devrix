...
func findLastAssistantWithToolCalls(msgs []types.Message) (types.Message, []ToolCallRef, bool) {
	for i := len(msgs) - 1; i >= 0; i-- {
		m := msgs[i]
		if m.Role != types.MessageRoleAssistant {
			continue
		}
		refs := ToolCallsFromAssistant(m)
		if len(refs) > 0 {
			return m, refs, true
		}
	}
...
func buildForkDirectiveUser(directive string, refs []ToolCallRef) types.Message {
	var b strings.Builder
...
func ForkPrefixFingerprint(msgs []types.Message) string {
	...
	for _, m := range msgs {
		if m.Role == types.MessageRoleAssistant {
			...
			if raw := m.Metadata[MetaToolCalls]; raw != "" {
				b.WriteString(raw)
			}
		}
