...
func TestBuildForkedMessages_should_use_identical_placeholder_results(t *testing.T) {
	...
	a := nested.BuildForkedMessages("scope auth module", parent)
	b := nested.BuildForkedMessages("scope billing module", parent)
	if len(a) != 2 || len(b) != 2 {
		t.Fatalf("expected 2 fork messages, got %d and %d", len(a), len(b))
	}

	fpA := nested.ForkPrefixFingerprint(a)
	fpB := nested.ForkPrefixFingerprint(b)
	if fpA != fpB {
		t.Fatal("fork prefix fingerprint should match for cache sharing")
	}
	if !containsAll(a[1].Content, nested.ForkPlaceholderResult, "scope auth module") {
		t.Fatal("fork A should contain placeholder and directive")
	}
	if !containsAll(b[1].Content, nested.ForkPlaceholderResult, "scope billing module") {
