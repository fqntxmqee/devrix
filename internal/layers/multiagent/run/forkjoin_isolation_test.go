...
func TestFork_metadata_writes_should_not_pollute_parent_session(t *testing.T) {
	f := newTestFactory(&run.StubEngine{Events: []*contracts.EngineEvent{{Type: "complete", Content: "ok"}}})
	...
	parentImpl, ok := parent.(*run.Impl)
	if !ok {
		t.Fatalf("parent is not *Impl")
	}
	...
	childImpl, ok := child.(*run.Impl)
	if !ok {
		t.Fatalf("child is not *Impl")
	}
...
func TestJoin_should_dedup_tool_call_ids(t *testing.T) {
	...
	c1.(*run.Impl).SetEngine(&run.StubEngine{Events: mkEvents("call_shared", "ok-1")})
	c2.(*run.Impl).SetEngine(&run.StubEngine{Events: mkEvents("call_shared", "ok-2")})
	c3.(*run.Impl).SetEngine(&run.StubEngine{Events: mkEvents("call_unique", "ok-3")})
